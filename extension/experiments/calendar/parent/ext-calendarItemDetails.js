...
this.calendarItemDetails = class extends ExtensionAPI {
  onLoadCalendarItemPanel(window, origLoadCalendarItemPanel, iframeId, url) {
    ...
    let panelFrame;
    if (window.tabmail) {
      panelFrame = window.document.getElementById(
        iframeId || window.tabmail.currentTabInfo.iframe?.id,
      );
    } else {
    ...
    panelFrame.contentWindow.addEventListener("load", (event) => {
      ...
      let areas = this.extension.manifest.calendar_item_details
        .allowed_areas || ["secondary"];
      ...
      if (areas.includes("secondary")) {
        ...
        tab.setAttribute(
          "label",
          this.extension.manifest.calendar_item_details.default_title,
        );
        tab.setAttribute("id", widgetId + "-calendarItemDetails-tab");
        tab.setAttribute(
          "image",
          this.extension.manifest.calendar_item_details.default_icon,
        );
        ...
        setupE10sBrowser(this.extension, browser, tabpanel, options).then(
          () => {
            const target = new URL(
              this.extension.manifest.calendar_item_details.default_content,
            );
        ...
        const browserRow = tabbox.appendChild(
          document.createElementNS("http://www.w3.org/1999/xhtml", "tr"),
        );
        const browserCell = browserRow.appendChild(
          document.createElementNS("http://www.w3.org/1999/xhtml", "td"),
        );
        browserRow.className = "event-grid-link-row";
        browserCell.setAttribute("colspan", "2");

        const separator = tabbox.appendChild(
          document.createElementNS("http://www.w3.org/1999/xhtml", "tr"),
        );
  ...
  onLoadSummary(window) {
    ...
    let areas = this.extension.manifest.calendar_item_details.allowed_areas || [
      "secondary",
    ];
    if (!Array.isArray(areas)) {
      areas = [areas];
  ...
  onStartup() {
    ...
    if (calendarItemDetails) {
      ...
      if (calendarItemDetails.default_icon) {
        calendarItemDetails.default_icon = this.extension.getURL(
          localize(calendarItemDetails.default_icon),
        );
      }

      if (calendarItemDetails.default_content) {
        calendarItemDetails.default_content = this.extension.getURL(
          localize(calendarItemDetails.default_content),
        );
      }
      if (calendarItemDetails.default_title) {
        calendarItemDetails.default_title = localize(
      ...
      if (
        Array.isArray(areas) &&
        areas.includes("inline") &&
        areas.includes("secondary")
      ) {
        throw new ExtensionError(
          "Cannot show calendar_item_details both inline and secondary",
        );
      }
    }
    ExtensionSupport.registerWindowListener(
      "ext-calendarItemDetails-summary-" + this.extension.id,
      {
        chromeURLs: ["chrome://calendar/content/calendar-summary-dialog.xhtml"],
        onLoadWindow: (window) => {
    ...
    ExtensionSupport.registerWindowListener(
      "ext-calendarItemDetails-event-" + this.extension.id,
      {
        chromeURLs: [
        ...
        onLoadWindow: (window) => {
          if (
            ...
            window.onLoadCalendarItemPanel = this.onLoadCalendarItemPanel.bind(
              this,
              window,
              orig.bind(window),
            );
